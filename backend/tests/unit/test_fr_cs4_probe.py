...
from app.dogfood.fr_cs4_probe import (
    FR_CS4_PROBE_MARKER,
...
def test_fr_cs4_probe_composed_skips_defect_when_root_false():
    with patch(
        "app.dogfood.fr_cs4_probe._fr_cs4_structural_root",
        return_value=False,
    ):
...
def test_fr_cs4_probe_composed_invokes_defect_when_root_true():
    with patch(
        "app.dogfood.fr_cs4_probe._fr_cs4_review_visible_defect",
    ) as mock_defect:
        with patch(
            "app.dogfood.fr_cs4_probe._fr_cs4_structural_root",
            return_value=True,
            ...
            assert fr_cs4_probe_composed() == FR_CS4_PROBE_MARKER
            mock_defect.assert_called_once_with("1 + 1")
