...
SELECT count(*)::int AS total_runs,
       ...
       count(*) FILTER (WHERE rr.error_message = 'superseded')::int AS superseded_error_runs
{join}
WHERE {where};
"""
...
SELECT count(*)::int AS total_jobs,
       ...
       count(*) FILTER (WHERE ij.status = 'superseded')::int AS superseded_jobs,
...
def _evaluate_rg15_gate(metrics: dict[str, Any], *, require_activity: bool) -> dict[str, Any]:
    ...
    if total_runs == 0 and total_jobs == 0:
        ...
        superseded_jobs = int(index.get("superseded_jobs") or 0)
        superseded_runs = int(review.get("superseded_error_runs") or 0)
        if superseded_jobs > 0 or superseded_runs > 0:
            add(
                "supersede_path",
                "PASS",
                f"superseded_jobs={superseded_jobs} superseded_runs={superseded_runs}",
            )
