...
Phase **D1** of [FINDING_RESOLUTION_POST_WAVE_C_GENERAL_PLAN.md](../FINDING_RESOLUTION_POST_WAVE_C_GENERAL_PLAN.md). Baseline: [backlog findings](../FINDING_RESOLUTION_POST_WAVE_C_BACKLOG_FINDINGS.md) § FR-CS4. **D1 only.**

**Goal:** Structural in-file fix **without** line-region overlap → group closes or judge resolves (FR-CS4 PASS).
...
- **Primary metric:** per-group `state` + `resolution_method` / judge outcome — not `pr_active_count` alone (VAL8).
- **Deploy boundary:** post–D0 deploy ISO in staging memo.
- **Staging prerequisite:** `judge_llm_enabled()` true on staging workers — Pass 3 is a no-op when false (`github_finding_closure.py:205–206`).
...
**Deliverable:** Table exists with post–D0 `--since` TBD row.

---

## D1.1 — Introduce structural probe (push 1)

**What:** Fixture module with defect on specific lines (Moonshot-visible); unit test imports fixture. Record group + revision ids after Revy.
...
**What:** Remove/refactor offending code **outside** finding line region (same file). Wait for Revy; record `resolution_status`, judge artifacts if any.

**Files:** probe fixture only (backend)

**Deliverable:** Memo row with `head_sha`, group id, `last_seen_revision_id`.

---

## D1.3 — Sign-off inspection

**What:** After publish on push 2: DB query group `state`, `resolution_method`, `resolution_status`; Revy resolution metrics block. Optional metrics:
...
**Deliverable:** FR-CS4 → **closed PASS** or documented blocker with group-level evidence.
...
| Group `state=resolved` + `resolution_method=verification_dismissed` (Pass 3 dismiss) | required |
| `judge_llm_enabled()` confirmed on staging | required |
