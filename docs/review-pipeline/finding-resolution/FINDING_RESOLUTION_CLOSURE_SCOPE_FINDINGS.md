...
**Execution plan:** [FINDING_RESOLUTION_CLOSURE_SCOPE_GENERAL_PLAN.md](./FINDING_RESOLUTION_CLOSURE_SCOPE_GENERAL_PLAN.md).
...
|----|-----|----------|--------|
| **FR-CS1** | Hygiene signals keyed to **push-pair** diff window only | **high** | open → wave C Track A |
| **FR-CS6** | Pass 2 (and Pass 3) use same pairing cohort filter as Pass 1 | **high** | open — **C1 blocker** |
| **FR-CS7** | No **HEAD-truth** path-gone signal (path absent at `head_sha`) | **high** | open → **CS-Q7 locked** |
| **FR-CS3** | Hygiene closures skew `resolution_rate_pct` via `_in_sync_stamp_cohort` | **medium** | open → C2 |
