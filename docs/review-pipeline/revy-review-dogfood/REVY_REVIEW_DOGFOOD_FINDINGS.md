...
| **RR-Q3** | Use external repo for staging probes? | **locked** | TenderPro #130 = **evidence source** only; RR-V validation on **`raimondskrauklis/revy`** PRs |
| **RR-Q4** | Block merge on 0% resolution rate? | **locked** | **Soft enable** — RR-V2 PASS on Revy PR #78; warn in summary; defer hard block on external repos |
