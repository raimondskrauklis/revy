...
| Check run **in progress** on PR | Greptile/Bugbot show spinner while reviewing | **G10:** `in_progress` at pipeline start (`start_pipeline_github_check`); `completed` at publish; reuses pipeline check id per `head_sha` | **shipped** (finalize-on-supersede → generation program) |
