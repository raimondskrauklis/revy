...
| 3 | `GITHUB_SURFACE_HARDENING_EXECUTION.md` | Execution index (after `create-execution-plan`) |
...
```text
findings → general plan → execution LOOP → dogfood row → merge (docs + code same PR)
```
