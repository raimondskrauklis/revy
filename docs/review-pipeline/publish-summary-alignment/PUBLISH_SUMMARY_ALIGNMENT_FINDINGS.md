...
| Push-delta resolution stamp | `apply_resolution_status_for_synchronize` `github_resolution_metrics.py:115+` (async via `github_tasks.apply_resolution_for_synchronize`; index enqueue after `applied` when non-coalesce) | G9 prose only |
