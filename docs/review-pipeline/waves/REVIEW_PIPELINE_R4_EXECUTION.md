...
- **Anthropic:** **not** the R4 primary — reserved for R5 judge / cross-family escalation (`ANTHROPIC_API_KEY`, `REVY_ANTHROPIC_MODEL=claude-sonnet-5`, `judge` queue). Optional `anthropic_review.py` scaffold in R4.2 for adapter reuse only.
