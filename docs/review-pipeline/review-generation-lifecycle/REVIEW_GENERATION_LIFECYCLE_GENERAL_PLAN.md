...
**Scope — in:** Debounced Celery task with `apply_async(countdown=…)` + `revision_id` token; revision row immediate; only pipeline enqueue delayed; stale task no-ops via `is_authoritative_for_pull_request_head` (**v1:** no `celery_task_id` column / no Redis); cap `review_coalesce_seconds` at 10; default **0**; `@revy review` + manual index bypass.
