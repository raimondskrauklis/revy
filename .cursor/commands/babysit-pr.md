...
3. **Fetch open Bugbot threads only** — GitHub GraphQL `reviewThreads` where `author.login == "cursor"` and `isResolved == false`. Read comment title, severity, path, line, description — not full raw JSON.
4. **Triage** — Fix only valid findings; skip false positives (say why). Ignore resolved/outdated threads.
5. **Checks** — if `backend/` Python touched: `pipenv run ruff check --fix .` then `pipenv run ruff check .` from `backend/`. Run targeted tests only when obvious (file has a colocated test).
...
fix: address Bugbot findings on PR #<n>
