...
- **SSOT (Moonshot + Greptile source):** `.greptile/review-context.json` — set `active_program`; add `programs[]` with `scope` + three doc paths (execution, findings, general plan)
- **Greptile:** regenerate `.greptile/files.json` — `cd backend && python -m scripts.generate_greptile_files_from_review_context --write` — **never hand-edit**
...
- [ ] **Code + docs in one PR:** first phase includes SSOT (`.greptile/review-context.json`) + generated `files.json` + `.cursor/BUGBOT.md` + `test_generate_greptile_files.py` gate — see `phase-execution` skill
